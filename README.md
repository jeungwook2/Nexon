...
## 서버구성
- 서버 gateway 는 3000번 포트를 사용했습니다.
- 서버 auth 는 3001번 포트를 사용했습니다.
- 서버 event 는 3002번 포트를 사용했습니다.

- gateway 서버로 모든 요청을 받아 라우팅을 수행했으며, 토큰 권한 검사를 진행하도록 설계했습니다.
...
- 진행하시면 자동으로 docker 에 이미지를 다운받아 컨테이너를 생성합니다.(3~7분정도 소요)
...
---
### JSON 형태 API 요청 방식
- Body 에 들어가는 부분(JSON)은 postman body안에 raw를 클릭하시고 복사 붙여넣기 진행해주시면 됩니다.
---
...
- value 값에는 Bearer 토큰값 (""는 제외하고 Bearer 과 토큰값 사이에 공백 1칸을 유지해주세요)
...
- 진행하신후 이벤트 목록 조회 후 상세 조회에서 확인하시면 보상 일차 day 값에 reward 배열 안에 해당 내용의 객체가 배열 안에 추가된 것을 확인하실 수 있습니다.
---
